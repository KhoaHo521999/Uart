...
def send():
    ...
    if not send_data:
        print("Sent Nothing")
    serial_object.write(str(send_data).encode())
    while True:
        try:
            print("Attempt to Read")
            readOut = serial_object.readline().decode('ascii')
            time.sleep(1)
...
if __name__ == "__main__":
    ...
    """
    # frames
    frame_1 = Frame(height=285, width=480, bd=3,
                    relief='groove').place(x=7, y=5)
    frame_2 = Frame(height=150, width=480, bd=3,
                    relief='groove').place(x=7, y=300)
    ...
    # progress_bars
    progress_1 = tkinter.ttk.Progressbar(
        orient=HORIZONTAL, mode='determinate', length=200, max=255)
    progress_2 = tkinter.ttk.Progressbar(
        orient=HORIZONTAL, mode='determinate', length=200, max=255)
    progress_3 = tkinter.ttk.Progressbar(
        orient=HORIZONTAL, mode='determinate', length=200, max=255)
    progress_4 = tkinter.ttk.Progressbar(
        orient=HORIZONTAL, mode='determinate', length=200, max=255)
    progress_5 = tkinter.ttk.Progressbar(
        orient=HORIZONTAL, mode='determinate', length=200, max=255)
    ...
    button_var = IntVar()
    radio_1 = Radiobutton(
        text="Windows", variable=button_var, value=1).place(x=10, y=315)
    radio_2 = Radiobutton(text="Linux", variable=button_var,
                          value=2).place(x=110, y=315)
    ...
    disconnect = Button(text="Disconnect",
                        command=disconnect).place(x=300, y=360)
